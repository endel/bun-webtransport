...
function createShieldMesh(device: pc.GraphicsDevice): pc.Mesh {
  ...
  for (let i = 0; i < numProfile - 1; i++) {
    const rCurr = frontStart + i * 2;
    const lCurr = frontStart + i * 2 + 1;
...
export function getShieldPickableMesh(device: pc.GraphicsDevice): pc.Mesh {
  if (!shieldPickableMesh) {
    shieldPickableMesh = createShieldMesh(device);
  }
...
export class TankEntity {
  ...
  constructor(private app: pc.AppBase, team: number) {
    ...
    if (tankModelContainer) {
      ...
      this.createBody(team);
      this.createTurret(team);
    }
  ...
  private createModelTank(team: number) {
    ...
    // Clone for body — hide turret parts
    ...
    // Clone for turret — hide body parts
  ...
  private createBody(team: number) {
  ...
  private createTurret(team: number) {
