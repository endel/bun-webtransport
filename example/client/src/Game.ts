...
import { Decoder, Callbacks, Reflection } from "@colyseus/schema";
import { TankEntity, getShieldPickableMesh, preloadTankModel } from "./Tank";
...
export class Game {
  ...
  // Pending patches (processed in animate loop)
  pendingPatches: Uint8Array[] = [];
  ...
  private async connect() {
    try {
      ...
      const certHashParam = params.get("certHash");

      const isLocal = /^(localhost|127\.)/.test(window.location.hostname);
      const serverHost = params.get("host") || (isLocal ? "127.0.0.1" : "echo.web-transport.dev");
      const serverPort = params.get("port") || (isLocal ? "4433" : "4434");
      const url = `https://${serverHost}:${serverPort}/battle`;

      this.connectStatus.textContent = "Connecting via WebTransport...";

      // Use cert hash pinning for self-signed certs (localhost/IP), skip for trusted CA domains
      const wtOptions: any = {};
      if (certHashParam) {
        wtOptions.serverCertificateHashes = [{
          algorithm: "sha-256",
          value: this.hexToBytes(certHashParam),
        }];
      } else if (/^(\d|localhost|127\.)/.test(serverHost)) {
        const resp = await fetch("/cert-hash");
        const hex = (await resp.text()).trim();
        wtOptions.serverCertificateHashes = [{
          algorithm: "sha-256",
          value: this.hexToBytes(hex),
        }];
      }
      ...
      // Datagram writer for input
      this.dgWriter = this.wt.datagrams.writable.getWriter();

      // Read state from incoming bidi stream
      this.readStateStream();

      this.wt.closed.then(() => {
        this.connected = false;
        this.connectStatus.textContent = "Disconnected";
  ...
  // ── Read framed messages from bidi stream ──────────────────
  private async readStateStream() {
    const t0 = performance.now();
    // Client opens the bidi stream (server sends state back on it)
    const stream = await this.wt!.createBidirectionalStream();
    console.log(`readStateStream: bidi stream opened (+${(performance.now() - t0).toFixed(0)}ms)`);
    ...
    try {
      while (true) {
        ...
        // Extract length-prefixed frames
        while (buffer.length >= 4) {
          ...
          if (messageIndex === 0) {
            // First frame: session ID
            this.mySessionId = new TextDecoder().decode(payload);
            console.log(`[+${(performance.now() - t0).toFixed(0)}ms] Session ID: ${this.mySessionId}`);
          } else if (messageIndex === 1) {
            // Second frame: reflection
            this.decoder = Reflection.decode(payload);
            this.state = this.decoder.state;
            this.bindStateEvents();
            console.log(`[+${(performance.now() - t0).toFixed(0)}ms] Reflection decoded`);
          } else if (messageIndex === 2) {
            // Third frame: full state
            this.pendingPatches.push(payload);
            console.log(`[+${(performance.now() - t0).toFixed(0)}ms] Full state received (${payload.length} bytes)`);
          } else {
            // Subsequent frames: state patches
            this.pendingPatches.push(payload);
          }
  ...
  // ── Bind schema callbacks (identical to original Colyseus client) ──
  private bindStateEvents() {
    const callbacks = Callbacks.get(this.decoder);

    // ── Tanks ──
    callbacks.onAdd("tanks", (tank: any, key: any) => {
      ...
      callbacks.listen(tank, "angle", (val: number) => {
        // Don't overwrite local player's turret aim
        if (key !== this.mySessionId) entity.targetAngle = val;
      });
      ...
      callbacks.listen(tank, "shield", (val: number) => {
        ...
        if (key === this.mySessionId) {
          const active = val > 0;
          this.shieldBar.style.display = active ? "block" : "none";
    ...
    callbacks.onRemove("tanks", (_tank: any, key: any) => {
    ...
    callbacks.onRemove("bullets", (_bullet: any, key: any) => {
    ...
    // ── Pickables ──
    callbacks.onAdd("pickables", (pick: any, key: any) => {
      const entity = this.createPickableEntity(pick);
      this.pickableEntities.set(key, entity);
    });

    callbacks.onRemove("pickables", (pick: any, key: any) => {
    ...
    // ── Teams ──
    callbacks.onAdd("teams", (team: any) => {
      callbacks.listen(team, "score", () => this.updateScores());
    });
  }

  // ── Pickable entity creation ───────────────────────────────
  private createPickableEntity(pick: any): pc.Entity {
  ...
  // ── HUD helpers ────────────────────────────────────────────
  private createTankHud(key: any): TankHud {
  ...
  private showWinnerScreen(winnerTeamId: number) {
    ...
    const myTank = this.state.tanks.get(this.mySessionId);
  ...
  private sendInput() {
    if (!this.dgWriter || !this.connected) return;

    const now = performance.now();
    if (now - this.lastInputTime < 50) return; // 20 Hz max
    ...
    buf[0] = MSG_INPUT;
    buf[1] = this.mouseDown ? 1 : 0;
    const inputView = new DataView(buf.buffer);
    inputView.setInt8(2, dirX);
    inputView.setInt8(3, dirY);
    inputView.setUint16(4, Math.round(aimAngle * 100) & 0xFFFF, true);

    this.dgWriter.write(buf).catch(() => {});
  }

  // ── Animation loop ─────────────────────────────────────────
  private animate(dt: number) {
    // Process buffered state patches in the update loop
    // (same timing as Colyseus patches — before render, never mid-frame)
    if (this.decoder && this.pendingPatches.length > 0) {
      for (const patch of this.pendingPatches) {
        this.decoder.decode(patch);
      }
